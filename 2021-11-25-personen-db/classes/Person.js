...
export default class Person {
  constructor({ id, name }) { // Hier werden aus dem übergebenen Parameter id und name herausgeladen; Der Parameter ist eigentlich so etwas: { id: 1, name: "Peter" }
    this.id = id;
    this.name = name;
  }
  ...
  static all() {
    return personsData.map(object => { // personsData ist eigentlich nur ein simples Array. Da wir haben ein Array mit `Person`en haben wollen, wandeln wir mit `map` das Array so für uns um.
  ...
  static find(id) {
    ...
    if (data) { // Wenn ein Datensatz gefunden wurde ...
  ...
  save() {
    personsData.push({ // fügt dem Datensatz-Array ein Eintrag hinzu
      id: this.id,
      name: this.name
    })
  }
